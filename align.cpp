...
int align(string &temp,string &sequence, vector<vector<int> > &cache)
{
	//calls++;
    ...
    temp.pop_back();
    sequence.pop_back();
    score_seq = align(temp, sequence, cache);
    ...
    vector<int> results = { match_result, delete_result, gap_result };
...
int align2(string &temp,string &sequence, vector<vector<int> > &cache, int get_at_least = TERRIBLE_SCORE)
{
	//calls++;
    ...
    temp.pop_back();
    sequence.pop_back();
    score_seq = align2(temp, sequence, cache, get_at_least - match_mismatch_score);
    int match_result = score_seq + match_mismatch_score;
    temp.push_back(temp_tail[0]);
    sequence.push_back(seq_tail[0]);
    ...
    sequence.pop_back();
    score_seq = align2(temp, sequence, cache, get_at_least - deletion_score);
    sequence.push_back(seq_tail[0]);
    int delete_result = score_seq + deletion_score;
    ...
    temp.pop_back();
    score_seq = align2(temp, sequence, cache,get_at_least - gap_score);
    temp.push_back(temp_tail[0]);
    int gap_result = score_seq + gap_score;
