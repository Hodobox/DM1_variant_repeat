...
if len(sys.argv) != 2:
    print('Usage: python3 $this $filename')
    exit(1)
...
def stack_hist(ax, stacked_data, sty_cycle, bottoms=None,
    ...
    for j in range(len(VALS)):
        ...
        for i in range(len(bottoms)):
            bottoms[i] = OFFSETS[repeats_plotted]
        repeats_plotted += 1
        top = bottoms + VALS[j]
        sty = STYS[j]
        sty.update(plot_kwargs)
...
for i in range(seqnum):
    params.append([int(x) for x in file.readline().strip().split()])
    
    if FILTER_ZEROS and 0 in params[-1]:
        continue
    
    OFFSETS = [ max([OFFSETS[i],params[-1][i]*len(patterns[i]) ]) for i in range(3) ]

for i in range(2,-1,-1):
    OFFSETS[i] = sum(OFFSETS[:i])
...
params = list(sorted(params,key = lambda x: -x[1]*len(patterns[1])))

#print(params)

skipped = 0
for i in range(len(params)):
    if FILTER_ZEROS and 0 in params[i]:
        skipped += 1
        continue
    ...
    for k in range(0,3):
        for _ in range(params[i][k]):
            for __ in range(len(patterns[k])):
                stack_data[k].append(i+1-skipped)

# set up histogram function to fixed bins
edges = np.linspace(0, seqnum+1-skipped, seqnum+1-skipped, endpoint=True)
...
plt.title(fname)
fig.savefig('Figures/masterplot/' + fname + '.png', dpi=300)
